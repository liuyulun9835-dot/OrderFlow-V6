...
**核心方法论（简述）**：
- 以**市场微结构/订单流（Order Flow）**为观测核心，结合 **HMM / TVTP-HMM / HSMM** 形成“**市场结构—状态—执行**”三层体系。
- 上层以 **Validator v2** 落实可证伪：单/多变量显著性、成本鲁棒（base/+50%/×2）、漂移监控；
- 工程采用 **双轨渐进（展开版）**：
  - **轨 A（低/中频｜Binance 主）**：
...
- 明确了 ATAS 数据窗口与精度限制（Bar≤3 个月、Tick≤7 天、L2≈1 天），以及 Binance 历史包长期可回溯的优势；
...
### 结尾提示（后续是否需要我补充的交付物）
- 若需要，我可继续输出：
  1) `orderflowV6_todo_v1.2.md`（按上述新增/强化卡片直接成文，已对齐格式）；
  2) `runbook_data_acquisition.md`（三源获取的操作清单 + 质量自检模板）；
  3) `merge_and_calibration_SOP.md`（重叠窗方法、阈值、报告模板、回滚条件）。
